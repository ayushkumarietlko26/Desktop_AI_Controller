...
class HandDetector:
        ...
        if self.max_num_hands == 2:
            ...
            if temp_result and len(temp_result) > 1:
                ...
                if dist_between_lms_x < 0.1:
                    ...
                    self.hands = self.mpHands.Hands(static_image_mode=self.mode, max_num_hands=1,
                                                    min_detection_confidence=self.min_detection_confidence,
                                                    min_tracking_confidence=self.min_tracking_confidence)
    ...
    def find_positions(self, img, hand_num=0):
        ...
        if self.results.multi_hand_landmarks:
            ...
            if hand_num == 0:
                self.position_history.append(landmark_positions[8][1:])

        return landmark_positions, handedness

    def get_swipe_direction(self, threshold=100):
        """
        Detects swipe direction based on position history.
